...
def GapFill(model, reactionsDB, solver, biomass_ouput, DB_type):
    ...
    Arguments:
        ...
        reactionsDB: str -- name of the constraint based model to be used
    ...
    if DB_type == 'txt':
        (model_extended, db_reactions) = extend_model_with_DB_plainText(model, reactionsDB)
    elif DB_type == 'sbml':
        (model_extended, db_reactions) = extend_model_with_DB_SBML(model, reactionsDB)
    else:
    ...
    for r_id in db_reactions:
        ...
        # add constraint (2)
        solver.add_constraint(r_id + '_lb', [(r_id, 1), (binary_variable, 10e6)], '>', 0, update_problem=False)
        # add constraint (3)
        solver.add_constraint(r_id + '_ub', [(r_id, 1), (binary_variable, -10e6)], '<', 0, update_problem=False)

    # add biomass production constraint (4)
    solver.add_constraint("r_biomass", [(model.detect_biomass_reaction(), 1)], '>', biomass_ouput, update_problem=False)

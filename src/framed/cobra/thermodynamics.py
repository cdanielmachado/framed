""" This module implements thermodynamics-based simulation methods.
...
def TFA(model, deltaG0, sdeltaG0=None, measured_concentrations=None, concentration_max=1e-2, concentration_min=1e-5,
        measured_fold_change=10, excluded=None, temperature=298.15, objective=None, minimize=False, constraints=None,
    ...
    Args:
        ...
        measured_concentrations (dict): measured metabolite concentrations (optional)
        ...
        measured_fold_change (float): allowable fold change of measured metabolites (default: 10)
    ...
    if not measured_concentrations:
    ...
    if not hasattr(solver, 'tFBA_flag'):
        ...
        for r_id in model.reactions:
            if r_id in deltaG0:
                ...
                solver.add_variable('dG_' + r_id, update_problem=False)
                dG0_min, dG0_max = deltaG0[r_id] - sdeltaG0[r_id], deltaG0[r_id] + sdeltaG0[r_id]
                solver.add_variable('dG0_' + r_id, dG0_min, dG0_max, update_problem=False)
                ...
                for m_id in model.reactions[r_id].stoichiometry:
                    if m_id not in excluded and m_id not in included:
                        ...
                        solver.add_variable('ln_' + m_id, ln_min_j, ln_max_j, update_problem=False)
        ...
        for r_id in model.reactions:
            if r_id in deltaG0:
                ...
                solver.add_constraint('ub_' + r_id, {r_id: 1, 'y_' + r_id: bigM}, '<', bigM, update_problem=False)
...
def TVA(model, deltaG0, sdeltaG0=None, measured_concentrations=None, concentration_max=1e-2, concentration_min=1e-5,
        measured_fold_change=10, excluded=None, temperature=298.15, constraints=None, ignore_model_bounds=True, reactions=None):
    ...
    for r_id in reactions:
        ...
        ub = sol.fobj if sol.status == Status.OPTIMAL else None
...
def NET(model, deltaG0, sdeltaG0=None, measured_concentrations=None, concentration_max=1e-2, concentration_min=1e-5,
    ...
    for r_id in model.reactions:
        if r_id in deltaG0:
            dG_min, dG_max = None, None
            if r_id in reaction_directions:
                if reaction_directions[r_id] < 0:
                    dG_min, dG_max = 0, None
                elif reaction_directions[r_id] > 0:
                    dG_min, dG_max = None, 0

            solver.add_variable('dG_' + r_id, dG_min, dG_max, update_problem=False)
            dG0_min, dG0_max = deltaG0[r_id] - sdeltaG0[r_id], deltaG0[r_id] + sdeltaG0[r_id]
            solver.add_variable('dG0_' + r_id, dG0_min, dG0_max, update_problem=False)

...
def simulate(model, time, n_steps=100, integrator_args=None):
    f = model.get_ODEs()
    f2 = lambda x, t: f(t, x)
    x0 = model.concentrations.values()
    t = linspace(0, time, n_steps)
    if not integrator_args:
        integrator_args = {'mxstep': 10000}
    X = odeint(f2, x0, t, **integrator_args)
    return t, X
...
def find_steady_state(model, endtime=1e9, abstol=1e-6):

    _, X = simulate(model, time=endtime, n_steps=2)
    x_ss = X[-1,:]
    v_ss = None
    f = model.get_ODEs()
    error = norm(f(0, x_ss))
